...
class HeroesRepositoryImpl: HeroesRepository {
    ...
    override suspend fun getAllHeroes(page: Int, limit: Int): HeroesResponse {
        return HeroesResponse(
            success = true,
            message = "ok",
            prevPage = calculatePage(page, limit)[PREV_PAGE_KEY],
            nextPage = calculatePage(page, limit)[NEXT_PAGE_KEY],
            heroes = getLimitedHeroesByPage(page, limit),
            lastUpdated = System.currentTimeMillis()
        )
    }

    private fun calculatePage(page: Int, limit: Int): Map<String, Int?> {
        val list = heroes.windowed(size = limit, step = limit, partialWindows = true)
        require(page <= list.size)
        val prevPage = if (page == 1) null else (page - 1)
        val nextPage = if (page == list.size) null else (page + 1)
        return mapOf(
    ...
    private fun getLimitedHeroesByPage(page: Int, limit: Int): List<Hero> {
        val list = heroes.windowed(size = limit, step = limit, partialWindows = true)
        require(page <= list.size)
        return list[page - 1]
    }
